...
export default function CalendarPage() {
  ...
  // Helper: heatmap intensity (more events = darker green)
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      cells.push(<div key={"empty" + i} className="p-1 h-20" />);
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      cells.push(
        <button
          ...
          className={`relative flex flex-col items-start gap-1 p-2 h-20 rounded-md transition-all duration-200 outline-none
            ${isToday ? "bg-green-50 ring-2 ring-green-300" : "hover:bg-gray-50"} ${getHeatClass(count)}`}
        >
          <div className="flex items-center w-full justify-between">
            <div className={`text-sm font-medium ${isToday ? "text-green-800" : "text-gray-800"}`}>{d}</div>
            {count > 0 && (
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 md:px-6 py-10">
      <div className="max-w-6xl mx-auto space-y-6">
              ...
              <div key={date} className="bg-white rounded-xl shadow-md border overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-sm text-green-900 font-semibold bg-green-50 px-2 py-0.5 rounded-full shadow">
                      {list.length}
                    </div>
                ...
                <div className="divide-y">
                  {list.map((v) => {
                    ...
                    return (
                      <div key={v.id} className="p-3 flex items-start justify-between hover:bg-gray-50 transition rounded-md">
        ...
        {selectedDate && (
          <div className="hidden md:block bg-white rounded-2xl shadow-md border p-4 mt-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm md:text-base font-semibold text-gray-900">Schedule for {selectedDate}</h2>
              <button
