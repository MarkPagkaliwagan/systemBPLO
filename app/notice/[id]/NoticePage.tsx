...
export default function NoticePage({ initialData }: Props) {
  ...
  const downloadApplicantPdf = async () => {
    ...
    // Small margins so it still looks clean
    const margin = 8;
    ...
    // Fit the whole form into ONE page
    ...
    const safeNoticeNo = (form.noticeNo || "notice")
      .trim()
      .replace(/[^\w\-]+/g, "_");

    pdf.save(`applicant-copy-${safeNoticeNo}.pdf`);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        try {
          await downloadApplicantPdf();
          openModal(
            "success",
            "Saved Successfully",
            "The notice has been submitted successfully, and the applicant copy PDF has been downloaded."
          );
        } catch (pdfError) {
          openModal(
            "warning",
            "Saved, but PDF Failed",
            "The notice was saved successfully, but the PDF download failed."
          );
        }
      } else {
