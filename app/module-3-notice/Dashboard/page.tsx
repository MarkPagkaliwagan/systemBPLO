...
export default function DashboardPage() {
  ...
  const handleRowClick = (violation: Violation) => {
    setSelectedRow(violation.id);

    // navigate to violation details page
    router.push(`/compliance/${violation.id}`);
  };
            ...
            <tbody className="divide-y divide-gray-200">
              {violations.map((v) => (
                <tr
                  ...
                  ${selectedRow === v.id ? "bg-green-100" : "hover:bg-green-50"}`}
                >
        ...
        <div className="md:hidden space-y-4">
          {violations.map((v) => (
            <div
              key={v.id}
              onClick={() => handleRowClick(v)}
              className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm cursor-pointer hover:bg-green-50"
            >
