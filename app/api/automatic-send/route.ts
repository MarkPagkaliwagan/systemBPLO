...
// Send email function — same as manual (with BCC support)
...
export async function GET() {
  try {
    ...
    for (const v of violations || []) {
      ...
      // Same as manual: interval must be set in DB
      if (interval === null || interval === undefined) continue;

      const lastSent = v.last_sent_time ? new Date(v.last_sent_time) : null;

      // 3️⃣ Check interval — same logic as manual
      if (lastSent && now < new Date(lastSent.getTime() + interval * 24 * 60 * 60 * 1000)) continue;
      ...
      switch (noticeLevel) {
        ...
        default:
          // All notices already sent, skip this violation
          continue;
      }
      ...
      try {
        ...
        continue; // Don't update DB if email failed
      }

      // 6️⃣ Update DB — same logic as manual
      ...
      if (noticeLevel < 2) {
        ...
        // Final notice sent, stop sending
